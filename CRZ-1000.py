...
def GA(filedir):
    ...
    for algNo in range(1, 9):
        ...
        if exportCSV:
            ...
            csvPathname = os.path.join(BASE_DIR, 'results', 'cap4', csvFilename)
            ...
            if utils.exist(pathname=csvPathname, overwrite=False):
                ...
                csvPathname = os.path.join(BASE_DIR, 'results', '100req', csvFilename)
            with open(csvPathname, 'w') as f:
                    # fieldnames = ['min_fitness', 'num_veh', 'avg_req', 'avg_dist', 'computing_time(s)']
                    fieldnames = ['min_fitness', 'num_veh', 'avg_req', 'avg_sharing', 'avg_dist', 'computing_time(min)', 'route']
    ...
    fig_path = os.path.join(BASE_DIR, 'results')
...
if __name__ == '__main__':
    ...
    pool.map(GA, filedir)
    total_computing_time = (timer() - start_time_0)/60
